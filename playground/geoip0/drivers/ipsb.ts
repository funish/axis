...
console.log(`\n--- Batch Lookup with IPv6 Preference ---`);
try {
  const options: QueryOptions = { version: "ipv6" };
  const results = await geoip.batchLookup(testIPs, options);
  if (!results) {
    console.log("No results returned from batch lookup");
  } else {
    console.log(`Found results for ${results.length} IPs with IPv6 preference`);

    results.forEach((result, index) => {
      console.log(
        `[${index + 1}] ${result.ip} -> ${result.country || "N/A"} (${result.timezone || "N/A"})`,
      );
    });
  }
} catch (error) {
